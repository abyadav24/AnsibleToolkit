...
from redfish.ris.utils import iterateandclear, diffdict, json_traversal, json_traversal_delete_empty
from redfish.ris.rmc_helper import IloResponseError, IdTokenError, InstanceNotFoundError, \
    InvalidPathError, InvalidSelectionError

from six.moves import input
from redfish.ris.utils import diffdict, navigatejson
from ilorest.rdmc_helper import ReturnCodes, RdmcError, InvalidCommandLineError, UI, \
    NoCurrentSessionEstablished, InvalidCommandLineErrorOPTS, InvalidFileInputError, \
    Encryption, NoChangesFoundOrMadeError, InvalidPropertyError, NoDifferencesFoundError
...
class EthernetCommand():
    ...
    def get_data(self, options):
        ...
        network_data_collection = eth_iface = manager_network_services = ilo_date_time = \
            OrderedDict()
        ...
        if self.eth_file and not get_only:
            if options.encryption:
                with open(self.eth_file, 'wb') as outfile:
                    outfile.write(Encryption().encrypt_file(json.dumps(outdata, indent=2,
    ...
    def configure_static_settings(self, data, options):
        ...
        if options.network_ipv4:
            ...
            if len(usr_data) > 2:
                data['IPv4Addresses'][0] = {'Address': usr_data[0],
                                            'Gateway': usr_data[1], 'SubnetMask': usr_data[2]}
                data['DHCPv4'].update({'DHCPEnabled': False})
                data['Oem'][self.rdmc.app.typepath.defs.oemhp]['DHCPv4'].update({'Enabled': False})
            else:
                raise InvalidCommandLineErrorOPTS("An invalid number of arguments provided to "
        ...
        if options.network_ipv6:
            ...
            if len(usr_data) > 2:
                next(iter(data['IPv6Addresses'])).update({'Address': \
                                                              usr_data[0], 'PrefixLength': usr_data[2]})
    ...
    def load_main_function(self, options, data=None):
        ...
        linking_dict = {'manager_interfaces': eth_type, 'systems_interfaces': eth_type,
                        'manager_network_services': 'networkprotocol.', 'ilo_date_time':
                            'datetime.'}
            ...
            try:
                if options.encryption:
                    with open(self.eth_file, 'rb') as file_handle:
                        data = json.loads(Encryption().decrypt_file(file_handle.read(),
                                                                    options.encryption))
                else:
                    with open(self.eth_file, 'rb') as file_handle:
        ...
        for ilotype, subsect in data.items():
            ...
            for _path in subsect:
                if not subsect[_path]:
                    continue
                elif 'ethernetinterface' in _type.lower() or \
                        'ethernetnetworkinterface' in _type.lower():
                    if 'managers' in _path.lower():
                        self.load_ethernet_aux(_type, _path, data[ilotype][_path])
                    elif 'systems' in _path.lower():
                        self.rdmc.ui.warn("Systems Ethernet Interfaces \'%s\' " \
                                ...
                                for _path in data[eth_config_type]:
                                    if 'managers' in _path.lower():
                                        try:
                                            ...
                                            self.load_ethernet_aux(eth_config_type, _path,
                                                                   data[eth_config_type][_path])
                                        except KeyError:
                                            self.rdmc.ui.printer("Unable to configure " \
                                                                 "\'UseNTPServers\' for \'%s\'.\n" % _path)
                                        self.rdmc.ui.printer('iLO must be reset in order for '
    ...
    def load_ethernet_aux(self, _type, _path, ethernet_data):
        ...
        if 'EthernetInterface' in _type:
            ...
            for curr_sel in self.rdmc.app.select(_type.split('.')[0] + '.', ("links/self/" +
                                                                             self.rdmc.app.typepath.defs.hrefstring,
        ...
        try:
            ...
            if ethernet_data.get('InterfaceEnabled') and not \
                    curr_sel.dict.get('InterfaceEnabled'):
            ...
            elif not ethernet_data.get('InterfaceEnabled') and not \
                    curr_sel.dict.get('InterfaceEnabled'):
                self.rdmc.app.patch_handler(_path, {"InterfaceEnabled": False}, silent=True)
                self.rdmc.ui.warn("NIC Interface Disabled. All additional configurations "
                                  "omitted.")
                return
        except (KeyError, NameError, TypeError, AttributeError):
            # check OEM for NICEnabled instead
            if not curr_sel.dict['Oem'][self.rdmc.app.typepath.defs.oemhp]['NICEnabled'] and \
                    ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp]['NICEnabled']:
                self.rdmc.app.patch_handler(_path, {"Oem": {self.rdmc.app.typepath.defs.oemhp: \
                                                                {"NICEnabled": True}}}, silent=True)
                self.rdmc.ui.printer("NIC Interface Enabled.\n")
            elif not curr_sel.dict['Oem'][self.rdmc.app.typepath.defs.oemhp]['NICEnabled'] and not \
                    ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp]['NICEnabled']:
                self.rdmc.app.patch_handler(_path, {"Oem": {
                    self.rdmc.app.typepath.defs.oemhp: {"NICEnabled": False}}}, silent=True)
        ...
        try:
            ...
            try:
                oem_dhcpv4curr = copy.deepcopy(curr_sel.dict['Oem']
                                               [self.rdmc.app.typepath.defs.oemhp]['DHCPv4'])
                oem_dhcpv4conf = copy.deepcopy(ethernet_data['Oem'] \
                                                   [self.rdmc.app.typepath.defs.oemhp]['DHCPv4'])
                ipv4curr = copy.deepcopy(curr_sel.dict['Oem'] \
                                             [self.rdmc.app.typepath.defs.oemhp]['IPv4'])
                ipv4conf = copy.deepcopy(ethernet_data['Oem'] \
                                             [self.rdmc.app.typepath.defs.oemhp]['IPv4'])
        ...
        try:
            ...
            try:
                ...
                oem_dhcpv6conf = copy.deepcopy(ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp] \
                                                   ['DHCPv6'])
                ipv6curr = copy.deepcopy(curr_sel.dict['Oem'][self.rdmc.app.typepath.defs.oemhp]['IPv6'])
                ipv6conf = copy.deepcopy(ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp]['IPv6'])
            except (KeyError, NameError):
        ...
        try:
            ...
            # try with OEM
            try:
                if oem_dhcpv4conf.get('Enabled') and not curr_sel.dict \
                        ['Oem'][self.rdmc.app.typepath.defs.oemhp]['DHCPv4']['Enabled']:
                    self.rdmc.app.patch_handler(_path, {'Oem': {
                        self.rdmc.app.typepath.defs.oemhp: {"DHCPv4": {"DHCPEnabled": True}}}},
                                                silent=True)
                    self.rdmc.ui.printer("DHCP Enabled.\n")
                    if 'IPv4Addresses' in ethernet_data:
                        del ethernet_data['IPv4Addresses']
                elif not oem_dhcpv4conf.get('Enabled') and curr_sel.dict \
                        ['Oem'][self.rdmc.app.typepath.defs.oemhp]['DHCPv4']['Enabled']:
        ...
        try:
            if "IPv4Addresses" in ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp]:
                spec_dict['Oem'][self.rdmc.app.typepath.defs.oemhp]['IPv4Addresses'] = \
                    copy.deepcopy(ethernet_data['Oem'][self.rdmc.app.typepath.defs.oemhp] \
                                      ['IPv4StaticAddresses'])
        except (KeyError, NameError, TypeError, AttributeError):
            pass
        ...
        try:
            ...
            try:
                if oem_dhcpv4conf.get('UseDNSServers'):
                    ...
                    json_traversal_delete_empty(data=ethernet_data,
        ...
        if errors and 'Virtual' not in ident_name:
            raise RdmcError("Ethernet configuration errors were found collectively on adapter: " \
    ...
    def patch_eth(self, _path, eth_data, errors=[]):
        ...
        try:
            if eth_data:
                ...
                raise NoDifferencesFoundError("No differences between existing iLO ethernet " \
            ...
            if hasattr(excp, 'message'):
                for key in excp.message[0]['MessageArgs']:
                    try:
                        ...
                        try:
                            drill_to_data(eth_data, list_o_keys=json_traversal(eth_data, key,
                                                                               ret_key_path=True))
                        except:
