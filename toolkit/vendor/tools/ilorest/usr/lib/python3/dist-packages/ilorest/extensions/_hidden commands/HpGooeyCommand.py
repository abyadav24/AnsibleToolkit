...
from ilorest.rdmc_helper import ReturnCodes, CommandNotEnabledError, InvalidCommandLineError, \
    InvalidCommandLineErrorOPTS, StandardBlobErrorHandler, \
    InvalidFileInputError, PartitionMoutingError, BirthcertParseError, \
    Encryption
...
class HpGooeyCommand():
    ...
    def remote_run(self, options):
        ...
        if options.write:
            if not (options.key and options.namespace):
                raise InvalidCommandLineError("Key and namespace are required"
                                              " for hpblob operations.")
            if not options.filename or not os.path.isfile(options.filename[0]):
                raise InvalidFileInputError("Given file doesn't exist, please "
                                            "provide a file with input data.")
            ...
            if not (errorcode == risblobstore2.BlobReturnCodes.SUCCESS or \
                        errorcode == risblobstore2.BlobReturnCodes.NOTMODIFIED):
    ...
    def local_run(self, options):
        ...
        if options.write:
            ...
            if options.key == 'birthcert':
                ...
                blobfiledata = self.writebirthcert(blobfiledata=blobfiledata,
                                                   blobdata=blobdata, )
    ...
    def remote_read(self, path):
        ...
        if resp.status == 200:
            ...
            raise StandardBlobErrorHandler('"remote read failure"')
        ...
        if labels:
            for label in labels:
                try:
                    ...
                    continue
                pumount = subprocess.Popen(['umount', path]
