...
from ilorest.rdmc_helper import ReturnCodes, CommandNotEnabledError, InvalidCommandLineError, \
            InvalidCommandLineErrorOPTS, InvalidOrNothingChangedSettingsError,\
            NoContentsFoundForOperationError, NoChangesFoundOrMadeError, \
            IncompatibleiLOVersionError, PathUnavailableError
...
class AutomaticTestingCommand():
    ...
    def __init__(self):
        self.ident = {
            ...
            'aliases': [],
            'auxcommands': ["LoginCommand", "LogoutCommand", "TypesCommand",
                            "SelectCommand", "GetCommand", "ListCommand", "InfoCommand",
                            "SetCommand", "StatusCommand", "CommitCommand", "SaveCommand",
                            "LoadCommand", "RawDeleteCommand", "RawHeadCommand",
                            "RawGetCommand", "RawPatchCommand", "RawPutCommand",
                            "RawPostCommand", "IscsiConfigCommand", "FirmwareUpdateCommand",
                            "IloResetCommand", "ISToolCommand", "ResultsCommand",
                            "BootOrderCommand", "ServerStateCommand", "VirtualMediaCommand",
                            "IloAccountsCommand", "IloFederationCommand",
                            "ClearRestApiStateCommand", "CertificateCommand",
                            "SigRecomputeCommand", "ESKMCommand", "SendTestCommand",
                            "SingleSignOnCommand", "ServerlogsCommand", "AHSdiagCommand",
                            "SMBiosCommand", "IloLicenseCommand", "DeleteComponentCommand",
                            "DownloadComponentCommand", "InstallSetCommand",
                            "ListComponentCommand", "UpdateTaskQueueCommand",
                            "UploadComponentCommand", "HpGooeyCommand",
                            "PendingChangesCommand", "ServerInfoCommand",
                            "MaintenanceWindowCommand", "FwpkgCommand", "RebootCommand",
                            "IPProfilesCommand", "FirmwareIntegrityCheckCommand",
                            "SetPasswordCommand", "BiosDefaultsCommand", "FwpkgCommand",
                            "ServerCloneCommand", "FactoryDefaultsCommand",
                            "DirectoryCommand", "SmartArrayCommand"]
        }
        self.cmdbase = None
        self.rdmc = None
    ...
    def run(self, line, help_disp=False):
        ...
        try:
            if sys.platform == 'darwin':
                raise CommandNotEnabledError("\'%s\' command is not supported on MacOS" % \
                                             str(self.name))
            elif 'VMkernel' in platform.uname():
                raise CommandNotEnabledError("\'%s\' command is not supported on VMWare" % \
                                             str(self.name))
        ...
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n")
        self.rdmc.ui.printer("****************************AUTOMATIC TESTING "
                             "STARTING***************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def helpmenusautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("**********************STARTING HELP MENU AUTOMATIC "
                             "TESTING***********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def loginautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("************************STARTING LOGIN AUTOMATIC "
                             "TESTING*************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def typesandselectautomatictesting(self, local, latestschema):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*******************STARTING TYPES AND SELECT "
                             "AUTOMATIC TESTING*******************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def getandsetautomatictesting(self, latestschema):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING GET AND SET AUTOMATIC"
                             " TESTING**********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
        ...
        if self.rdmc.app.typepath.flagiften:
            biosinputlist = ["Attributes/BootMode", "Attributes/MissingItem",
                             "Attributes/AdminName", "Attributes/AdminEmail",
                             "Attributes/AdminPhone", "Attributes/CustomPostMessage",
                             "Attributes/ServerAssetTag", "Attributes/ServerName",
                             "Attributes/ServerOtherInfo", "Attributes/ServerPrimaryOs",
                             "Attributes/ServiceEmail", "Attributes/ServiceName",
                             "Attributes/ServiceOtherInfo", "Attributes/ServicePhone"]
        else:
            biosinputlist = ["BootMode", "MissingItem", "AdminName",
                             "AdminEmail", "AdminPhone", "CustomPostMessage",
                             "ServerAssetTag", "ServerName", "ServerOtherInfo",
                             "ServerPrimaryOs", "ServiceEmail", "ServiceName",
                             "ServiceOtherInfo", "ServicePhone"]
                ...
                if getresults and isinstance(getresults[next(iter(getresults))], dict):
                    ...
                    for key, value in list(getresults.items()):
                        if key == 'BootSourceOverrideTarget':
                            ...
                            randstring = ''.join(random.SystemRandom().\
                    ...
                    for key, value in list(getresults.items()):
                        randstring = ''.join(random.SystemRandom().choice(string.ascii_letters + \
                    ...
                    InvalidCommandLineError("Variable '%s' was not found in the " \
                                                    "current selection.\n" % item)
    @log_decor
    def getandinfoautomatictesting(self, item="", latestschema=False, getval=None,
                                   already_checked=[]):
        ...
        if isinstance(getdata, dict):
            for key, val in list(getdata.items()):
                ...
                self.getandinfoautomatictesting(item=key, latestschema=latestschema, getval=val,
            ...
            for kii in getdata:
                ...
                for key, val in list(kii.items()):
                    ...
                    if key in already_checked:
                        ...
                        already_checked.append(key)
                    self.getandinfoautomatictesting(item=key, latestschema=latestschema,
                                                    getval=val, already_checked=already_checked)
    @log_decor
    def saveandloadautomatictesting(self, local, latestschema):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("********************STARTING SAVE AND LOAD "
                             "AUTOMATIC TESTING*********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")

        biosinputlist = ["AdminName", "AdminEmail", "AdminPhone",
                         "CustomPostMessage", "ServerAssetTag", "ServerName",
                         "ServerOtherInfo", "ServerPrimaryOs", "ServiceEmail",
    ...
    def iscsiautomatictesting(self):
        ...
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n")
        self.rdmc.ui.printer("*****************STARTING ISCSI CONFIGURATION "
                             "AUTOMATIC TESTING******************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def rawautomatictesting(self, local):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING RAW COMMANDS "
                             "AUTOMATIC TESTING*********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
        ...
        try:
            ...
            for line in testfile:
                ...
                if '"MySession":' in line and '"MySession": false' not in line \
                                        and '"MySession": true' not in line:
                    next_item = True
...
#                            keep_going = True
    ...
    def istoolautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING ISTOOL "
                             "AUTOMATIC TESTING***************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def resultsautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING RESULTS "
                             "AUTOMATIC TESTING**************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def bootorderautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING BOOT ORDER "
                             "AUTOMATIC TESTING***********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
...
#             self.rdmc.ui.printer("Entry is the current boot setting.\n")
    ...
    def bootorderhelper(self):
        ...
        self.auxcommands['login'].run("")
        results = self.rdmc.app.get_handler(self.rdmc.app.typepath.defs.systempath,
    ...
    def serverstateautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************SERVER STATE "
                             "AUTOMATIC TESTING******************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def virtualmediaautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************VIRTUAL MEDIA "
                             "AUTOMATIC TESTING*****************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
        return
        ...
        if not results["Inserted"] or not results["Oem"]\
                            [self.rdmc.app.typepath.defs.oemhp]["BootOnNextServerReset"]:
            raise InvalidOrNothingChangedSettingsError("VM not found.")

        self.auxcommands['virtualmedia'].run("2 --remove")

        results = self.rdmc.app.get_handler(
            '/rest/v1/Managers/1/VirtualMedia/2', service=True, silent=True).dict
    ...
    def accountautomatictesting(self):
        ...
        errors = []
        priv_dict = {"LoginPriv": 1, "RemoteConsolePriv": 2, "UserConfigPriv": 3,
                     "iLOConfigPriv": 4, "VirtualMediaPriv": 4, "VirtualPowerAndResetPriv": 5,
                     "HostNICConfigPriv": 6, "HostBIOSConfigPriv": 7, "HostStorageConfigPriv": 9,
                     "SystemRecoveryConfigPriv": 7}
        for i in range(5):
            randname = ''.join(random.SystemRandom().choice(string.ascii_letters + \
            ...
            try:
                ...
                if 'rand_priv' in locals():
                    errors.append("Unable to create account \'{}\',  " \
                                  "\'{}\': {}".format(randname, acct_str, str(excp)))

            results = self.rdmc.app.get_handler(self.rdmc.app.typepath.defs.accountspath,
                                                service=True, silent=True).dict[\
                                             self.rdmc.app.typepath.defs.collectionstring]

            if 'Id' not in list(results[0].keys()):
                newresults = []

                for acct in results:
                    acct = self.rdmc.app.get_handler(acct[self.rdmc.app.typepath.defs.\
            ...
            for acct in results:
                if acct['Oem'][self.rdmc.app.typepath.defs.oemhp]['LoginName'] != randname:
                    ...
                    try:
                        ...
                        if 'rand_priv' in locals():
                            errors.append("Unable to modify privilege \'{}\' for account " \
                                          "\'{}\': {}".format(rand_priv, randname, str(excp)))
                        else:
                            errors.append("Unable to retrieve privileges for account \'{}\' " \
                                          ": {}".format(randname, excp))
                    break
        ...
        self.auxcommands['logout'].run("")
        if errors:
            raise Exception("The following errors occurred while testing accounts: %s" \
                            % errors)
    ...
    def federationautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************ADDFEDERATION "
                             "AUTOMATIC TESTING*****************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
        ...
        newresults = []
        for fed in results:
            fed = self.rdmc.app.get_handler(fed[\
                 self.rdmc.app.typepath.defs.hrefstring], service=True, silent=True).dict
    ...
    def serverlogstesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************STARTING SERVERLOGS "
                             "AUTOMATIC TESTING***********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def ahsdiagtesting(self, local):
        ...
        if local and os.name != 'nt':
            self.rdmc.ui.printer("\n*********************************************"
                                 "************************************\n")
            self.rdmc.ui.printer("*********************STARTING AHSDIAG "
                                 "AUTOMATIC TESTING**************************\n")
            self.rdmc.ui.printer("***********************************************"
                                 "**********************************\n\n")
            pass
            ...
            self.auxcommands['ahsdiag'].run("--WriteMarkerPost --instance 1 --markervalue"
                                            " 3 --markertext \"Automatictesting\"")
    ...
    def crapistesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************CLEARRESTAPI "
                             "AUTOMATIC TESTING******************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def certificateautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*******************SSO AND TLS CERTIFICATE "
                             "AUTOMATIC TESTING*********************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def sigrecomputeautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************SIGRECOMPUTE "
                             "AUTOMATIC TESTING******************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def sendtestautomatictesting(self):
        ...
        try:
            ...
            self.auxcommands['set'].run(oem+'/AlertMailEmail=test@test.test '+oem+'/AlertMailEnabled=True '\
                            +oem+'/AlertMailSMTPServer=testserver.test.test '\
                            +oem+'/AlertMailSenderDomain=testdomain.test.test --commit')
        except:
            pass
        ...
        try:
            #setup for syslog
            self.auxcommands['select'].run(self.rdmc.app.typepath.defs.managernetworkservicetype)
            self.auxcommands['set'].run(oem+'/RemoteSyslogServer=testserver.test.svrtest '
                            +oem+'/RemoteSyslogEnabled=True --commit')
        except:
            pass
    ...
    def ssoautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*********************SINGLESIGNON "
                             "AUTOMATIC TESTING******************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def ilolicensetesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("***********************ILOLICENSE "
                             "AUTOMATIC TESTING******************************\n")
        self.rdmc.ui.printer("***************************************************"
                             "******************************\n\n")
    ...
    def installsettesting(self):
        ...
        for installset in installsetlist:
            try:
                ...
                self.rdmc.ui.printer("An error occured attempting to remove the "\
                                     "file: %s, logged: %s\n" % (installsetfile, excp))
                continue
...
#             self.auxcommands['rawpost'].run("session_payload.json --getheaders --service")
            ...
            for client in self.rdmc.app._rmc_clients:
                ...
                break
            self.auxcommands['rawpost'].run("sut_provider_registration_payload.json " \
                              "--getheaders --response --service --providerid" \
                              "=SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_registry_payload.json --getheaders --response" \
                 " --service --providerid=SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_schema.json --getheaders --response --service "\
                             "--providerid=SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_settings_schema.json --getheaders --response "\
                     "--service --providerid=SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_tasksettings_schema.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " + session_key)
            self.auxcommands['rawput'].run("sut_systeminventory_schema.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " + session_key)
...
#             compsigname = "yyy"
            ...
            for client in self.rdmc.app._rmc_clients:
                ...
                session_key = " --sessionid={0}".format(session_key)
                break
            self.auxcommands['rawdelete'].run("<session URL returned by previous rawpost> " \
                                "--getheaders --response --service --providerid=" \
                                "SUT-PROVIDER" + session_key)
            self.auxcommands['rawpost'].run("sut_provider_registration_payload_gen10.json " \
                              "--getheaders --response --service --providerid=" \
                              "SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_registry_payload_gen10.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " \
                             + session_key)
            self.auxcommands['rawput'].run("sut_schema_gen10.json --getheaders --response " \
                     "--service --providerid=SUT-PROVIDER" + session_key)
            self.auxcommands['rawput'].run("sut_settings_schema_gen10.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " \
                             + session_key)
            self.auxcommands['rawput'].run("sut_tasksettings_schema_gen10.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " \
                             + session_key)
            self.auxcommands['rawput'].run("sut_systeminventory_schema_gen10.json --getheaders " \
                             "--response --service --providerid=SUT-PROVIDER " \
                             + session_key)
...
#         self.auxcommands['downloadcomp'].run("Component uri returned by the rawget above")
    ...
    def maintenancewindowautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("******************MAINTENANCE WINDOW "
                             "AUTOMATIC TESTING***************************\n")
        self.rdmc.ui.printer("***********************************************"
                             "**********************************\n\n")
        ...
        for entry_ in range(0, 10):
            ...
            maintenancewindow_str = "add " + maintenancewindow_start_time + \
                " --expire=" + maintenancewindow_end_time + \
                    " --name=TestMaintenanceEntry" + str(entry_) + \
                    " --description " + maintenancewindow_descr
    ...
    def serverinfoautomatictesting(self):
        ...
        info_list = {"--fans", "--processor", "--memory", "--thermals", "--power", "--system"}
        info_string = "--fans --processor --memory --thermals --power --system --showabsent"
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*************************SERVER INFO AUTOMATIC "
                             "TESTING***************************\n")
        self.rdmc.ui.printer("***********************************************"
                             "**********************************\n\n")
    ...
    def fwpkgcommandautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("************************FIRMWARE PACKAGE AUTOMATIC "
                             "TESTING***********************\n")
        self.rdmc.ui.printer("*************************************************"
                             "********************************\n\n")
    ...
    def ipprofilesautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("*****************************IP PROFILES AUTOMATIC "
                             "TESTING***********************\n")
        self.rdmc.ui.printer("*************************************************"
                             "********************************\n\n")
    ...
    def fwintegritycheckautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("******************FIRMWARE INTEGRITY CHECK AUTOMATIC "
                             "TESTING*********************\n")
        self.rdmc.ui.printer("*************************************************"
                             "********************************\n\n")
    ...
    def setbiospasswordautomatictesting(self, local):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("**********************SET BIOS PASSWORD AUTOMATIC "
                             "TESTING************************\n")
        self.rdmc.ui.printer("***********************************************"
                             "**********************************\n\n")
    ...
    def setbiosdefaultsautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("************************SET BIOS DEFAULTS AUTOMATIC "
                             "TESTING**********************\n")
        self.rdmc.ui.printer("***********************************************"
                             "**********************************\n\n")
        ...
        try:
            ...
            self.rdmc.ui.printer(repr(self.auxcommands['biosdefaults'].\
                                                                run("--manufacturingdefaults")))
        except:
    ...
    def smartarrayautomatictesting(self):
        ...
        self.rdmc.ui.printer("\n*************************************************"
                             "********************************\n")
        self.rdmc.ui.printer("************************SMART ARRAY AUTOMATIC "
                             "TESTING*******************\n")
        self.rdmc.ui.printer("***********************************************"
                             "********************************\n\n")
        ...
        if content:
            tmp = self.auxcommands['smartarray'].selection_output(dict(controller=None), content)
        else:
        ...
        if isinstance(data, dict):
            for _, d_file in list(data.items()):
                try:
                    ...
                    errors.append("An error occured attempting to remove the file: %s, logged: %s\n" % \
                ...
                if 'ComputerSystem.' in type:
                    for path in list(data[type].keys()):
                        ...
                        if 'AssetTag' in data[type][path]:
                            data[type][path]['AssetTag'] = 'AutomaticTestAsset'.join(random.\
    ...
    def automatictesting_helper_fb(self, url, components=False):
        ...
        try:
            ...
            read_url = read_url[read_url.find('<table>'):read_url.find('<table>')+\
                              read_url.find('</table>')]
            while read_url.find('<tr>') > 0:
                current_str = read_url[prev_tr:prev_tr + read_url.find('</tr>')]
                current_str = current_str[current_str.find('<a href'):\
                                current_str.find('<a href')+\
                                current_str.find('</a>')]
                current_str = current_str[current_str.find('=\"')+2:\
                                current_str.find('=\"')+2 +\
                                current_str.find('\">')]
                current_str = current_str[:current_str.find(">")-1]
                if current_str.split('/')[-1] in ignore_list or \
                    current_str.split('/')[-2] in ignore_list:
                    ...
                    for url in table_row_list:
                        ...
                        if last_element.count('.') > 2:
                            ...
                            if 'compsig' in last_element:
                                if components_dict.get(compname) is not None \
                                    and compname in last_element:
                                    components_dict[compname].append(url)
                                    table_row_list.remove(url)
                                    continue
                            if 'part' in last_element and '_' in last_element:
                                (split_compname, _) = compname.split('_')
                                if split_compname in components_dict and url \
                                    not in components_dict[split_compname]:
