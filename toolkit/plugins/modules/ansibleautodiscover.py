...
def resolve_ipv4_address(ipv6_address, device_info=None):
    ...
    try:
        ...
        # Method 4: Try known working IP ranges but with better filtering
        log_debug("Trying known working IP ranges with correlation...")
        known_ranges = [
            ("172.23.55", range(90, 120)),    # Based on successful discoveries
            ("192.168.1", range(100, 110)),   # Common IPMI range
            ("192.168.0", range(100, 110)),   # Alternative IPMI range
        ]
        
        for base_network, host_range in known_ranges:
            log_debug(f"Scanning range {base_network}.{min(host_range)}-{max(host_range)}")
            for host in host_range:
                ...
                try:
                    ...
                    if ping_result.returncode == 0:
                        # Try to correlate this IP with our IPv6 device
                        if correlate_ipv4_with_ipv6(test_ip, ipv6_address, device_info):
                            log_debug(f"Found correlated IPv4 {test_ip} for {ipv6_address}")
                            return test_ip
                except Exception:
                    continue
        
        log_debug(f"Could not resolve IPv4 for {ipv6_address}")
        return None
...
def find_ipv4_by_serial_correlation(target_serial, ipv6_address):
    """Find IPv4 address by querying devices and matching serial numbers"""
    try:
        log_debug(f"Looking for device with serial: {target_serial}")
        
        # Try common IP ranges
        test_ranges = [
            ("172.23.55", range(90, 120)),
            ("192.168.1", range(100, 110))
        ]
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if device_serial == target_serial:
                                    log_debug(f"Found matching serial {device_serial} at {test_ip}")
                                    return test_ip
                        except:
                            pass
                except:
